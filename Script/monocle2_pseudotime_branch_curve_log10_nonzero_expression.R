...
# ========== A. 普通拟时序（所有分支合并，去除零表达细胞） ==========
for (gene_id in names(genes_to_plot)) {
  gene_sym <- genes_to_plot[gene_id]
  min_e    <- min_expr_tbl[gene_sym]
  
  raw_expr <- as.numeric(exprs(mycds_final[gene_id, ]))
  keep_idx <- raw_expr > 0  # 去除掉 0 表达的细胞
  
  p_gene <- plot_genes_in_pseudotime(
    mycds_final[gene_id, keep_idx],
    color_by  = "cluster_stage3",
    min_expr  = min_e,
    cell_size = 2
  ) +
    scale_color_manual(values = base_col) +
    ggtitle(paste0(gene_sym, " (nonzero only)")) +
    scale_y_continuous(trans = "log10",
                       breaks = trans_breaks("log10", function(x) 10^x),
                       labels = trans_format("log10", math_format(10^.x)))
  
  ggsave(file.path(opt$outdir, paste0(gene_sym, "_pseudotime_nonzero.png")),
         plot = p_gene, width = 6, height = 4.5, dpi = 300)
  ggsave(file.path(opt$outdir, paste0(gene_sym, "_pseudotime_nonzero.pdf")),
         plot = p_gene, width = 6, height = 4.5, device = cairo_pdf)
}

# ========== B. 两个分支对比（实线/虚线，去除零表达细胞） ==========
plot_gene_two_directions <- function(cds, gene_id, gene_sym,
  ...
  raw_expr <- as.numeric(exprs(cds[gene_id, ]))
  keep_idx <- raw_expr > 0   # 去除掉 0 表达的细胞
  
  df <- data.frame(
    Pseudotime = pd$Pseudotime[keep_idx],
    Expr       = raw_expr[keep_idx] + 1,   # 不再 log2，直接用原始+1
    State      = pd$State[keep_idx],
    Cluster    = pd$cluster_stage3[keep_idx],
  ...
  p <- ggplot(df, aes(Pseudotime, Expr)) +
    ...
    labs(title = paste0(gene_sym, " (branched pseudotime, nonzero only)"),
...
# 只对 WRKY9 和 Kasl 去掉零表达细胞做分支拟时序
for (gene_id in c("LOC107864105")) {
  gene_sym <- genes_to_plot[gene_id]
  min_e    <- min_expr_tbl[gene_sym]
  ...
  ggsave(file.path(opt$outdir, paste0(gene_sym, "_branched_nonzero.png")),
         plot = p_gene, width = 6, height = 4.5, dpi = 300)
  ggsave(file.path(opt$outdir, paste0(gene_sym, "_branched_nonzero.pdf")),
         plot = p_gene, width = 6, height = 4.5, device = cairo_pdf)
}

cat("✅ 基因拟时序曲线 (普通版+去零 + Branched去零，log10纵坐标) 导出完成；结果保存在：", opt$outdir, "\n")
